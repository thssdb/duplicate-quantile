...
public class DyadicSpaceSavingForDupli {
  ...
  private long N;
  private int maxItemNum, exactTopLayer, botLayerMaxItem; // [exactTopLayer , LOG_U) are exact layers.
...
//    System.out.println("\t\tInitial DSS.\t maxItemNum:"+maxItemNum+"\texact_layer:"+(LOG_U-exactTopLayer)+"\tbotLayerMaxItem:"+botLayerMaxItem);
  }
  ...
  void updateSpaceSaving(Long2LongOpenHashMap mapV2C,Long2ObjectAVLTreeMap<LongOpenHashSet> mapC2Vs,long v){
...
//    System.out.println("\t\t"+dataToLong(-1.0)+"\t"+dataToLong(1.0)+"\t"+(+dataToLong(-1.0)<dataToLong(1.0))+"\t"+Double.doubleToLongBits(-1)+"\t"+Double.doubleToLongBits(1));
    while(L<R){
      long mid=L/2+R/2;
      if(L+2>=R)mid=L;
//      System.out.println("\t\tBinary Search\t"+L+"\t"+R+"\t"+mid+"\t\tvalid:"+(L<=mid&&mid<=R)+"\tmidV:"+(longToResult(mid)));
      if(getRank(mid)>=q*N)R=mid;else L=mid+1;
    }
