...
//    System.out.println("\t\t\t??!!"+1.0*(maxBufferNum*8+maxEntryNum*24)/maxMemoryByte);
  ...
  public DoubleArrayList query(DoubleArrayList phis) {
    ...
    for(Tuple entry:entries)maxGD=Math.max(maxGD,entry.g+entry.delta);
    long targetErr = maxGD/2;
    LongArrayList ranks = new LongArrayList();for(double phi:phis)ranks.add( (long) (phi * (N - 1)) + 1);
    ...
    for (Tuple entry : entries) {
      gSum += entry.g;
      maxRank = gSum + entry.delta;
      while (pos<ranks.size()&&maxRank - targetErr <= ranks.getLong(pos) && ranks.getLong(pos) <= gSum + targetErr) {
         ans.add(entry.v);
         pos++;
      }
    }
...
//    System.out.println();
    ...
    for(int i=entryNum-2;i>=1;i--){
      now=entries.get(i);
      if(now.g+last.g+last.delta<removalThreshold && now.delta>=last.delta) // band: compare log(2εN-delta) is to compare delta..
        last.g+=now.g;
      else{
        revEntries.add(last);
        last=now;
  ...
  private void compress() {// |entries|<=11/(2ϵ)log_2(2ϵN) ; We may need to increase ε to make sure |entries| is limited.
//    System.out.print("\t\t"+rankAccuracy+"\t\t-->\t\t");
//    while(true){
//      int entryNumUpperBound=(int)(11.0/(2*rankAccuracy)*Math.log(2*rankAccuracy*(N+bufferNum))/Math.log(2));
...
//      System.out.print("\t\t\tN:" + N + " entry:" + entryNum + "\t" + 1.0 * entryNum / maxEntryNum + "\tThreshold:\t" + (2 * rankAccuracy *N) + "\t\tval(g+delta,cap,band):\t");
//      for (Tuple entry : entries) System.out.print("\t" + entry.v + "(" + (entry.g + entry.delta) +","+((2 * rankAccuracy *N)-entry.delta)+","+(int)Math.floor(Math.log((2 * rankAccuracy *N)-entry.delta+1)/Math.log(2)) + ")");
//      System.out.println();
      rankAccuracy*=1.1;
      compress();
    }
  ...
  public void show(){
      System.out.print("N:"+N+"\tε:"+rankAccuracy+"\tentry:"+entryNum+"\tfull:"+1.0*entryNum/maxEntryNum+"\t\tval(g+delta,cap,band):\t");
      int removalThreshold=(int)(2*rankAccuracy*N);
      for (Tuple entry : entries) System.out.print("\t" + entry.v + "(" + (entry.g + entry.delta) +","+(removalThreshold-entry.delta)+","+(int)Math.floor(Math.log(removalThreshold-entry.delta+1)/Math.log(2)) + ")");
