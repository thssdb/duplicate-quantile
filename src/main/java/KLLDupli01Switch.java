...
public class KLLDupli01Switch extends KLLSketchForQuantile {
  ...
  final int BitsPerItem=64/*+1*/; // TODO

  public KLLDupli01Switch(int maxMemoryByte) {
    N = 0;
...
//        if (dupliPairInLevel.getInt(i) * 2 >= getLevelSize(i) / 2)
          if (dupliPairInLevel.getInt(i)>=2)
...
//      System.out.println("\t\t\thashMap:" + 2 * 8 * HashCommon.arraySize(newFreqValueCount.size(), Long2LongOpenHashMap.DEFAULT_LOAD_FACTOR) + "Byte");
        ...
        for (int lv = cntLevel - 1; lv >= 0; lv--)
          for (int i = levelPos[lv + 1] - 1; i >= levelPos[lv]; i--)
            if (!dupli[i] || !newFreqValueCount.containsKey(num[i])) {
              cntPos--;
            }
        if(cntPos<0){
          continue;
...
//          System.out.println("\t\t!!!WARN\t delete non-freq item:\t");
        ...
        if (checkN != N)
          System.out.println("\t\t\t\tchecking N:\t N:" + N + "\t\tcheckN:" + checkN);
