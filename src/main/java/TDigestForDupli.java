...
//    this.compression = this.clusterNumMemLimit / 11; //  cluster:buffer = 1:10
  ...
  public TDigestForDupli(int maxMemByte, int mergingBuffer) {
    this(maxMemByte);
    this.compression = this.clusterNumMemLimit / (1+mergingBuffer); //  cluster:buffer = 1:mergingBuffer
  }
...
//    System.out.println("\t\t expectedClusterSize:"+expectedClusterSize + " totN:"+totN+" oldClusterNum:"+oldClusterNum);
    for (int i = 0; i < oldClusterNum; i++) {
      if(compression!=this.compression&&oldClusterNum-i+cntClusterNum+(cnt.count>0?1:0)<=maxSeriByte/ElementSeriByte){ // for serialize.
...
//    System.out.println("\t\t add: "+(value));
  ...
  public void merge(List<TDigestForDupli> anotherList) {
    for(TDigestForDupli another:anotherList)
      merge(another);
  }
...
//    return flag1&&flag2;
  ...
  private long fastPossibleSizeLEValue(double v){
    ...
    if(index==-1)return 0;else return preCountSum[index];
  }
  ...
  private long fastPossibleSizeGEValue(double v){
    ...
    if(index==cluster.size())return 0;else return sufCountSum[index];
  }
