...
public class DDSketchPositiveForDupli implements Serializable {
  ...
  public static int getBucketNumLimit(int seriByte){return (int)(seriByte/bucketNumPerByteInMemoryForApprox/COEFFICIENT);}
...
//    System.out.println("\t\tDD collapse.\t"+positive_buckets.size()+"\t"+1.0*positive_buckets.size()/threshold_for_compression);
  }
  ...
  private double getR(int index){
    return index>DIVIDE_HALF?Math.pow(gamma, index-DIVIDE_DELTA):(index==DIVIDE_HALF?positive_collapse_bound:-Math.pow(gamma, index-1));
  }
